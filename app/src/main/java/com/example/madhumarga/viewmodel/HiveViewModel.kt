...
class HiveViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addHive(hive: Hive) {
        viewModelScope.launch {
            ...
            val hiveId = repository.insert(hive).toInt()

            val log = HiveLog(
                hiveId = hiveId,
                inspectionNotes = "Hive inspection completed",
                treatmentAdministered = "",
                feedingDetails = "",
                healthStatus = if (hive.activityLevel == "Low")
    ...
    fun getSuggestion(hive: Hive): String {
        return when {
            ...
            hive.activityLevel == "Low" || hive.pestsSeen ->
                "Intervention Needed"

            hive.honeyCollected > 50 ->
                "Ready for Harvest"
